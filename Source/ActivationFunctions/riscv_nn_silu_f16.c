...
int32_t riscv_nn_silu_f16(const float16_t * in_vec, uint32_t size, float16_t * out_vec)
{
    ...
    {
		float32_t sigmoid = 0;
		x = (float32_t) in_vec[i];
        float32_t num = exp_f32(x);
        float32_t den = num + cst_1;
        sigmoid = num / den;
		y = x * sigmoid; // silu(x) = x*sigmoid(x).
        out_vec[i] = (float16_t) y;
    }
#else
    //--- const values for sigmoid ---
    const float16_t cst_1 = (float16_t) 1.f;
	uint32_t i;
    float16_t x, y;
    for(i = 0 ; i < size ; i++)
    {
