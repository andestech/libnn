...
int32_t riscv_nn_reduce_sum_s8(const int8_t * in_tensor,
                               ...
                               const uint8_t axis,
                               const int32_t in_offset, //[-127, 128]
                               const int32_t out_shift,
                               const int32_t out_scale,
                               const int32_t out_offset, //[-128, 127]
                               ...
                               int8_t * out_tensor)
{
    ...
    int32_t input_stride = 1, input_stride0 = 1, input_stride1 = 1, input_stride2 = 1;
        ...
        for (int32_t i_in_i1 = 0; i_in_i1 <= end_i1; i_in_i1++)
        {
            for (int32_t i_in_i2 = 0; i_in_i2 <= end_i2; i_in_i2++)
            {
                ...
                for(int32_t j=0; j <= reduced_dim; j++)
                {
                    ...
                    sum += (in_tensor[i_src] + in_offset);
                }

                int32_t sum_out = riscv_nn_requantize_s64((int64_t)sum, reduced_scale, out_shift);
                sum_out += out_offset;
                sum_out = MAX(sum_out, act_min);
                sum_out = MIN(sum_out, act_max);
                out_tensor[i_dst++] = (int8_t)sum_out;
            }
        }
    }
    return 0;
