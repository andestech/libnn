        ...
        for(int32_t in_w = -pad_x, out_w = 0, ker_h_start = MAX(0, -in_h); out_w < out_tensor_dim_x; in_w += stride_x, ++out_w)
        {
            for(int32_t in_ch = 0, out_ch = 0, ker_w_start = MAX(0, -in_w); out_ch < out_tensor_ch; ++in_ch, out_ch += ch_mult)
            {
                for(int mult_tile = 0; mult_tile < ch_mult; mult_tile += 4)
                {
                    ...
                    out_buff0 = riscv_nn_requantize(out_buff0, out_scale[out_ch + 0 + mult_tile], out_shift[out_ch + 0 + mult_tile]);
                    out_buff1 = riscv_nn_requantize(out_buff1, out_scale[out_ch + 1 + mult_tile], out_shift[out_ch + 1 + mult_tile]);
                    out_buff2 = riscv_nn_requantize(out_buff2, out_scale[out_ch + 2 + mult_tile], out_shift[out_ch + 2 + mult_tile]);
                    out_buff3 = riscv_nn_requantize(out_buff3, out_scale[out_ch + 3 + mult_tile], out_shift[out_ch + 3 + mult_tile]);
