    ...
    const int32_t dilation_y)
{
    if ((dilation_x == 1)
        ...
        && ((in_tensor_ch * ker_dim_x * ker_dim_y) <= 512))
    {
        return riscv_nn_conv_HWC_s16_s16_s8_asym_bias_any_get_buffer_size(in_tensor_ch,
                ker_dim_x, ker_dim_y);
    }
    else
    {
        return riscv_nn_conv_HWC_s16_s16_s8_asym_bias_fast_any_get_buffer_size(in_tensor_ch,
                ker_dim_x, ker_dim_y);
    }
}
