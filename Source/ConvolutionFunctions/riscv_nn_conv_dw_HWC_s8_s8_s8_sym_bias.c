...
void conv_dw_HWC_s8_s8_s8_sym_bias_chmult(const q7_t * in_tensor,
                ...
                for (i_ch_mult = 0; i_ch_mult < ch_mult; i_ch_mult++)
                {
                    ...
                    for (i_ker_y = 0; i_ker_y < ker_dim; i_ker_y++)
                    {
                        for (i_ker_x = 0; i_ker_x < ker_dim; i_ker_x++)
                        {
                            ...
                            {
                                conv_out += in_tensor[(in_row * in_tensor_dim + in_col) * in_tensor_ch + i_ch_in] *
                                            ker_weight[(i_ker_y * ker_dim + i_ker_x) * (in_tensor_ch * ch_mult) + idx_out_ch];
                            }
                        }
                    }
                    conv_out = (conv_out >> pre_rshift) * out_scale + NN_ROUND(post_rshift);
                    out_tensor[(i_out_y * out_tensor_dim + i_out_x) * out_tensor_ch + idx_out_ch] = NDS_ISA_SATS((conv_out >> post_rshift), 8);
                }
            }
        }
    }
...
void conv_dw_HWC_s8_s8_s8_sym_bias(const q7_t * in_tensor,
        const uint16_t in_tensor_dim,
        const uint16_t in_tensor_ch,
        const q7_t * ker_weight,
        const uint16_t out_tensor_ch,
                                        ...
                                        q15_t * in_tmp_buf)
{
    ...
    {
        conv_dw_HWC_s8_s8_s8_sym_bias(in_tensor, in_tensor_dim, in_tensor_ch,
            ker_weight, out_tensor_ch, ker_dim, pad, stride, bias, pre_rshift,
            out_scale, post_rshift, out_tensor, out_tensor_dim, in_tmp_buf);
    }
    else
    {
        conv_dw_HWC_s8_s8_s8_sym_bias_chmult(in_tensor, in_tensor_dim,
            in_tensor_ch, ker_weight, out_tensor_ch, ker_dim, pad, stride,
            bias, pre_rshift, out_scale, post_rshift, out_tensor,
