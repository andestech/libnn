                                                      ...
                                                      int8_t *tmp_buf)
{
    ...
    {
        riscv_nn_set_zero_s8((int8_t*)out_tmp_buf, sizeof(int32_t) * out_tensor_dim_y * out_tensor_dim_x * out_tensor_ch);
                        ...
                        for (int i_ker_x = 0; i_ker_x < ker_dim_x; i_ker_x++)
                        {
                            for (int i_out_ch = 0; i_out_ch < out_tensor_ch; i_out_ch++)
                            {
                                ...
                                if ((i_out_x >= 0) && (i_out_x < out_tensor_dim_x) && (i_out_y >= 0) && (i_out_y < out_tensor_dim_y))
                                {
                                    ...
                                    out_tmp_buf[(i_out_y * out_tensor_dim_x + i_out_x) * out_tensor_ch + i_out_ch] += (in_val * wt_val);
                                }
                            }
                        }
                    }
        ...
        for (int i_out_y = 0; i_out_y < out_tensor_dim_y; i_out_y++)
        {
            for (int i_out_x = 0; i_out_x < out_tensor_dim_x; i_out_x++)
            {
                for (int i_out_ch = 0; i_out_ch < out_tensor_ch; i_out_ch++)
                {
                    ...
                    acc = (acc >> pre_rshift) * out_scale + NN_ROUND(post_rshift);
                    out_tensor[i_out_ch + (i_out_y * out_tensor_dim_x + i_out_x) * out_tensor_ch] = NDS_ISA_SATS((acc >> post_rshift), 8);
                }
            }
        }
